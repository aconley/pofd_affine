...
int main(int argc, char** argv) {
  ...
  // Change number of walkers based on number of procs;
  //  given the way the affine invariant MCMC works, there's
  //  little point in not stepping up to the next multiple
  //  of 2 * (nproc-1)
  int nslaves2 = 2 * (nproc - 1);
  int nexcess = nwalkers % nslaves2;
  if (nexcess != 0) {
    nwalkers += nslaves2 - nexcess;
    std::cout << "Increased number of walkers to " << nwalkers
              << " to better make use of the " << (nproc - 1)
              << " slave nodes." << std::endl;
  }
