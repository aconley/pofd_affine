...
int main(int argc, char **argv) {
  ...
  char optstring[] = "hfH2:3:n:N:s:S:vVw:";
  while ( ( c = getopt_long(argc,argv,optstring,long_options,
			    &option_index ) ) != -1 ) 
    switch(c) {
    case 'h' :
      std::cerr << "NAME" << std::endl;
      std::cerr << "\tpofd_affine_getDoublePD -- get the P(D1,D2) for a" 
		<< std::endl;
      std::cerr << "\tuser specified model" << std::endl;
      std::cerr << std::endl;
      std::cerr << "SYNOPSIS" << std::endl;
      std::cerr << "\tpofd_affine_getDoublePD [options] initfile beamfile1 "
		<< "beamfile2" << std::endl; 
      ...
      std::cerr << "\tWrites out the P(D) for the model in initfile using"
		<< std::endl;
      std::cerr << "\ta spline plus log-normal color model."
		<< std::endl;
      std::cerr << std::endl;
      ...
      std::cerr << std::endl;
      std::cerr << "\tBeamfile1 and beamfile2 are the beams in FITS format." 
		<< std::endl;
      std::cerr << std::endl;
      ...
      std::cerr << "\t\tMaximum flux value in band 1. (def: Largest knot minus the"
		<< std::endl;
      std::cerr << "\t\tmean flux from the model)" << std::endl;
      std::cerr << "\t\tMaximum flux value in band 2."
		<< std::endl;
      std::cerr << "\t-n, --nflux value" << std::endl;
      std::cerr << "\t\tThe number of requested fluxes along each dimension; "
		<< "also " << std::endl;
      std::cerr << "\tthe FFT length. Should be a power of 2. (def: 2048)"
		<< std::endl;
      std::cerr << "\t-N, --nedge value" << std::endl;
      std::cerr << "\t\tSize of edge inteprolation. (def: 256)." << std::endl;
      std::cerr << "\t-s, --sigma1 noise" << std::endl;
      std::cerr << "\t\tThe assumed per-pixel noise, band 1 (assumed Gaussian, "
		<< "def: 0.002)" << std::endl;
      std::cerr << "\t-S, --sigma2 noise" << std::endl;
